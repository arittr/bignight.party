...
function getPRInfo(branch: string): PRInfo | null {
	try {
		const json = execQuiet(
			`gh pr view --head "${branch}" --json number,state,baseRefName`,
		);
...
async function mergePR(
	...
	if (!prInfo) {
		...
		exec(
			`gh pr create --head "${branch}" --base "${parent}" --title "${branch}" --fill`,
		);
	} else {
		...
		if (prInfo.baseRefName !== parent) {
			...
			console.log(
				`  Updating PR base: ${branch} (${prInfo.baseRefName} → ${parent})`,
			);
...
function showHeader(config: Config): void {
	...
	console.log(
		`  Auto-merge: ${config.auto ? "yes (wait for CI)" : "no (immediate)"}`,
	);
	console.log(
		"════════════════════════════════════════════════════════════════════════════",
	);
...
function showStackPreview(stack: Branch[], trunk: string): void {
	...
	console.log(
		`  Stack Preview (${stack.length} branches, bottom → top → ${trunk})`,
	);
	console.log(
		"════════════════════════════════════════════════════════════════════════════",
	);
