	...
	try {
		...
		if (!fetchResult) {
			throw new WikipediaAPIError(
				`Failed to fetch Wikipedia page: ${pageTitle}`
			);
		}
		...
		if (infobox) {
			...
			const dateText = typeof dateField === "object" && dateField !== null && "text" in dateField
		...
		const description = firstSentence && typeof firstSentence === "object" && "text" in firstSentence
	...
	for (const key of keys) {
		...
		if (typeof value === "string") {
			if (
				...
				nomination.isWinner = value.toLowerCase().includes("won") ||
