...
import type { EventCreateInput } from "@/schemas/event-schema";
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      {createEvent.error && (
        <div className="p-3 bg-red-50 border border-red-200 rounded-md">
          <p className="text-sm text-red-600">
            {createEvent.error instanceof Error ? createEvent.error.message : "Failed to create event"}
          </p>
        </div>
      )}
      ...
      <div className="flex gap-3 pt-4">
        ...
        <button
          ...
          disabled={isSubmitting || createEvent.isPending}
          onClick={() => router.push(routes.admin.events.index())}
          type="button"
        >
          Cancel
        </button>
