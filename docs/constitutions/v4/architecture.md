...
### oRPC Procedures
- Type-safe remote procedure calls using contract-first pattern
- **MANDATORY**: Use `implement(contract)` from `@orpc/server`
- Contracts defined with `oc.input().output()` from `@orpc/contract`
- Authentication and authorization via `.use(authenticatedProcedure)` chaining
- Input validation with Zod schemas (in contract layer)
- Call Service layer for business logic
- **Architecture**: Contract-first API design with openRPC standard
- Full type inference without manual `any` annotations
...
- ✅ Services
- ✅ oRPC contracts from `src/lib/api/contracts/`
- ✅ `implement(contract)` pattern (MANDATORY)
- ✅ Auth context via `.use(authenticatedProcedure)`
- ✅ Models (for simple CRUD)
- ❌ Direct Prisma imports (call models instead)
- ❌ Ad-hoc procedures with `{ input }: any` (breaks type inference)
